...
public class AssignRegistrationToUnitUseCase {
    ...
    @Transactional
    public UUID execute(UUID competitionUnitId, UUID registrationId, Integer laneOrPosition) {
        if (!competitionUnitRepository.existsById(competitionUnitId)) {
            throw new BusinessException("results.competition_unit_not_found", "Заплыв/забег не найден");
        }

        try {
            registrationLookup.getById(registrationId);
        } catch (IllegalArgumentException exception) {
            throw new BusinessException("results.registration_not_found", "Заявка не найдена");
        }

        if (entryRepository.existsByCompetitionUnitIdAndRegistrationId(competitionUnitId, registrationId)) {
            throw new BusinessException("results.entry_already_exists", "Участник уже назначен в этот заплыв");
        }

        CompetitionUnitEntryEntity entry = new CompetitionUnitEntryEntity();
